...
호환 v3 archive는 ZIP container에 strict `manifest.json`, `document.json`, `images/<imageId>.rgba`, 선택적 `thumbnail.png`를 기록한다. 현재 기본 writer와 Recovery는 v4를 생성한다. RGBA bytes는 JSON에 넣지 않는다. archive entry 수·이름·압축 해제 크기·전체 크기·symlink와 path traversal을 검사한다.
...
Migration은 반드시 v1→v2→v3→v4 순서다. v1→v2는 빈 palette를 추가한다. v2→v3는 기존 Layer의 `imageId`를 첫 Frame의 Cel로 이동하고 기본 100ms Frame, 빈 Tag record, Cel index를 만든다. v3→v4는 RGBA Pixel Layer tree, normal blend, indexed/tilemap용 빈 collection을 만든다. v1/v2/v3 reader는 유지되지만 writer와 Recovery는 항상 v4를 생성한다.
