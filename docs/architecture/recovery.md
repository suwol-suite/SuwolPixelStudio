...
Autosave는 dirty revision만 1.5초 debounce 후 v3 archive, strict metadata, 최대 128px 첫 Frame thumbnail로 분리 저장한다. Main은 `userData/recovery` 아래 고정 파일명과 atomic write만 사용하며 Renderer가 경로나 generic filesystem API를 받지 않는다.
